...
For our app, we used AI to create datasets by asking ChatGPT to create a script that would generate prompt completion JSON files from Kaggle datasets. These JSON files would then be converted into JSONL files that were used to fine-tune our AI.

During this project, we encountered various limitations with AI. One major challenge was ensuring consistent replies generated by the AI and managing the cost of fine-tuning. In our fitness plan generation feature, the AI generates exercise names, and corresponding workout gifs are displayed. However, we faced an issue when the AI assigned different names to the same exercise, resulting in a chance of no appropriate gif being retrieved. To overcome this, we had to resort to redundancy by associating the same gif with multiple exercise names.

Another limitation was the cost associated with fine-tuning the AI. Originally, we used OpenAI's Davinci-3 model and planned to train it with 1800 prompts. However, due to cost constraints, we had to reduce this number to 500 prompts. Consequently, the AI's capabilities were limited to the prompts it was trained on, and it couldn't effectively answer queries outside of its training domain. To optimize costs, we decided to upgrade from Davinci-3 to ChatGPT 3.5.

Throughout the development of this app we used ChatGPT to troubleshoot various errors and fix various bugs. We also used it to create prompts and completion sets to fine-tune our AI features.

## Known Bugs & Limitations

- Generating diet and workout plans sometimes take longer than 1-2 minutes.
- Calorie tracker displayed on the Home page does not remain in its updated state once the page is re-rendered.

## Features for Future

- Implement interactivity features for the calendar on the Calendar page, so that users can modify their upcoming events.
- Implement nodemailer so that adding/deleting a friend will become a proper friend request.
