...
For our app, we used AI to create datasets by asking ChatGPT to create a script that would generate prompts and complete JSON files from Kaggle datasets. These JSON files would then be converted into JSONL files that were used to fine-tune our AI.

During this project, we encountered various limitations with AI. One major challenge was ensuring consistent replies generated by the AI and managing the cost of fine-tuning. In our fitness plan generation feature, the AI generates exercise names, and corresponding workout gifs are displayed. However, we faced an issue when the AI assigned different names to the same exercise, resulting in a chance of no appropriate gif being retrieved. To overcome this, we introduced redundancy by associating the same gif with multiple exercise names.
